...
function wolfram2desmos(input) {
	...
	while (findFinal(/[\/\^\_\√∛%](?!\()/g) != -1) {
		...
		while (true) {
			...
			if (bracket == -1) {
				...
				if (input[i] == " " || input[i] == "," || input[i] == ":") { // eg: "a/(a 2" → "a/(a) (2)"
					insert(i, ")");
					break;
				}
				if (i == input.length) {
	...
	while (reciprocalExponents2Surds && findFinal(/\^\{\\frac\{1\}\{[A-z\d\s\.\	+\-\*]*\}\}/g) != -1) {
		...
		if (input[i] == ")") {
			...
			while (i > 0) {
				...
				if (bracket == 0) {
					...
					overwrite(i, "{");
					insert(i, "√[" + selection.match(/(?<=\^\{\\frac\{1\}\{)[A-z\d\s\.\	+\-\*]*(?=\}\})/)[0] + "]");
					break;
				}
			}
		}
	...
	while (find(/(?<=Ⓩ)\(/) != -1) {
		...
		while (i < input.length) {
			...
			if (input[i] == "," && bracket == -1) {
				...
				input = input.slice(0, startingIndex + 1) + input.slice(i,	input.length);
